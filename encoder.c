...
ISR(INT0_vect)
{
    //same as above but new in ISR-reintroduced to continue not initalized (states introduced from past lb of encoder)
    uint8_t x = PIND;
    a = (x >> PD2) & 1;
    b = (x >> PD3) & 1;
    ...
    if (new_state != old_state) //makes sure it has changed values - from logic of previous lab
    {
