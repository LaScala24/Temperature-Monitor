...
void ds_write1bit(void)
{
    ...
    DDRC |= (1 << PC1);        //goes low for 2us at start of each
    _delay_us(2);             

    DDRC &= ~(1 << PC1);       //remains high for 60us
    _delay_us(60);             
...
void ds_write0bit(void)
{
    ...
    DDRC |= (1 << PC1);        //hold low for 60us
    _delay_us(60);            
    DDRC &= ~(1 << PC1);       

    _delay_us(2);              //delay another 2us before starting
...
unsigned char ds_readbit(void)
{
    unsigned char SendIT;

    DDRC |= (1 << PC1);     //pulls it low and holds it for 2us
    _delay_us(2);            
    
    DDRC &= ~(1 << PC1);    //pulls it high and waits 10us then reads

    _delay_us(10);           

    if ((PINC & (1 << PC1)) == 0)
    {
        SendIT = 0; //sends in a 0
    }
    else
    {
        SendIT =1; //sends in a 1
    }

    _delay_us(50);           //waits an additional 50us to complete
    return SendIT;
}
